...
function generateResults
{
	...
	db -db as400 -query \${ select trim(K00001)||'|'||substr(trim(F00002), length(trim(F00002))-5,4)||'-'||substr(trim(F00002), length(trim(F00002))-1,2)||'|'|| trim(substr(trim(F00002),length(trim(F00002))-40,30))||'|'||substr(trim(F00002),1,15)||'|'|| substr(hex(F00002),32,11)||'|'||substr(hex(F00002),44,11)  from "GL.JRNL" where K00001 like 'JV %' and F00002 like '%$thisYear%' and K00001 not like '%-%' and K00001 not like '%\_%' order by substr(trim(F00002), length(trim(F00002))-1,2) asc }$ $ipDef $libDef $usrDef $usrPwd
EOF
}
